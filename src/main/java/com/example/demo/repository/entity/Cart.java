...
public class Cart extends AggregateRoot {
    ...
    public void addProduct(Product product, ProductOption option, int quantity) {
        if (option.getStock() < quantity) {
            throw new IllegalStateException("상품 재고가 부족하여 장바구니에 담을 수 없습니다.");
        }
        // 상품/옵션이 장바구니에 있는지 검증
        items.stream()
                ...
                .ifPresentOrElse(
                        ...
                        () -> items.add(new CartItem(product, option, quantity)) // 없으면 카트에 추가
                );
        }

        public long calculateTotalPrice() { return items.stream().mapToLong(CartItem::calculatePrice).sum(); }
    }
